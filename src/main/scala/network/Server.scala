...
object Server {
  def start(session: ArrayBuffer[Session] with mutable.SynchronizedBuffer[Session], port: Int, backlog: Int, address: String) {
    ...
    actor {
      while (true) {
        ...
        actors.Actor.actor {
          ...
          session += Session(socket, is, os, is.readLine())
        }
      }
    }
  }
