...
//      var sessionKey: SessionKey = SessionKey(1)
...
//
//                    val Ta = message.substring(message.indexOf("ta:") + 3, message.indexOf("ka:")).toLong
//                    networkManager.addMessage("get Ta from " + sessionName)
//                    networkManager.addMessage(Ta.toString)
//                    if (new Date().getTime - Ta > 50000) {
...
//                    networkManager.getUser.sessionKey = sessionKey.key
...
//                    val publicKeyMSG = new Message(message.substring(message.lastIndexOf("}") + 1))
...
//                }
//                else {
//                  Gamma.decrypt(new Message(message).getBytes, sessionKey.key)
